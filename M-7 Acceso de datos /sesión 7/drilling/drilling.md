...
from nombre archivo import ..., ....
from producto.models import Producto, Fabrica

productos = Producto.objects.all().values() # SELECT * FROM producto_producto;
fabricas = Fabrica.objects.all().values() SELECT * FROM producto_fabrica; -> # este es un comentario

* 2. Obtenga los campos de nombre, precio, y fecha de vencimiento de los productos. Demuestretambién cuál es la consulta SQL que se genera del ORM.
